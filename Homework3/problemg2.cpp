...
struct Allocations * initAllocationMap()
{
    ...
    allocMap = (struct Allocations *)malloc(sizeof(struct Allocations));
...
struct ProcessMap * initProcMap()
{
    ...
    pMap = (struct ProcessMap *)malloc(sizeof(struct ProcessMap));
...
struct NeighborRanks * calcNeighbors(int rank, struct ProcessMap * pMap)
{
    ...
    neighbors = (struct NeighborRanks *)malloc(sizeof(struct NeighborRanks));
...
struct ChunkHalos * initHalos(int rowRange, int colRange)
{
    ...
    halos = (struct ChunkHalos *)malloc(sizeof(struct ChunkHalos));
    ...
    halos->eHalo  = (int *)malloc(sizeof(int) * halos->eHaloSize);
    halos->wHalo  = (int *)malloc(sizeof(int) * halos->wHaloSize);
    halos->nHalo  = (int *)malloc(sizeof(int) * halos->nHaloSize);
    halos->sHalo  = (int *)malloc(sizeof(int) * halos->sHaloSize);
    // Cardinal directions are scalars
    halos->neHalo = (int *)malloc(sizeof(int));
    halos->nwHalo = (int *)malloc(sizeof(int));
    halos->seHalo = (int *)malloc(sizeof(int));
    halos->swHalo = (int *)malloc(sizeof(int));
...
void exchangeHalos(int ** subWorld, struct ChunkHalos * halos, struct NeighborRanks * neighbors)
{
    ...
    int * toNBuffer,
        ...
        * toEBuffer,
        * toWBuffer;
    toNBuffer = (int *)malloc(sizeof(int) * halos->nHaloSize);
    toSBuffer = (int *)malloc(sizeof(int) * halos->sHaloSize);
    toEBuffer = (int *)malloc(sizeof(int) * halos->eHaloSize);
    toWBuffer = (int *)malloc(sizeof(int) * halos->wHaloSize);
...
void updateSubWorld(int ** subWorld, struct ChunkHalos * halos, struct ProcessChunkInfo * pcInfo)
{
    ...
    worldCopy = (int **)malloc(sizeof(int *) * (pcInfo->rowRange + 2));

    for(i = 0; i < pcInfo->rowRange + 2; i++)
    {
        worldCopy[i] = (int *)malloc(sizeof(int) * (pcInfo->colRange + 2));
    }
    ...
    // Perform update on world
    Kokkos::parallel_for("worldUpdate", pcInfo->rowRange, KOKKOS_LAMBDA(int i)
    {
        int j;
    ...
    // Transfer copy of world to actual world
    Kokkos::parallel_for("worldCopy", pcInfo->rowRange, KOKKOS_LAMBDA(int i)
    {
        int j;
        for(j = 1; j < pcInfo->colRange + 1; j++)
        {
            subWorld[i + 1][j] = worldCopy[i + 1][j];
        }
    });
...
void aggregateSubWorlds(int world[WORLD_HEIGHT][WORLD_WIDTH], struct ProcessMap * pMap)
{
    ...
    for(i = 0; i < P; i++)
    {
        for(j = 0; j < Q; j++)
        {
            ...
            tempSubWorld = (int *)malloc(sizeof(int) * subWorldHeight * subWorldWidth);
...
void forwardSubWorld(int ** subWorld, struct ProcessChunkInfo * pcInfo)
{
    ...
    flatWorld = (int *)malloc(sizeof(int) * (pcInfo->rowRange) * (pcInfo->colRange));
